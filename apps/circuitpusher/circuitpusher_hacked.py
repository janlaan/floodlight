...
parser.add_argument('--add', dest='action', action='store_const', const='add', default='add', help='action: add, delete, hack')
parser.add_argument('--delete', dest='action', action='store_const', const='delete', default='add', help='action: add, delete, hack')
parser.add_argument('--hack', dest='action', action='store_const', const='hack', default='add', help='action: add, delete, hack')
...
if args.action=='add':
    ...
    command = "curl -A HackDemoApplication -s http://%s/wm/device/?ipv4=%s" % (args.controllerRestIp, args.srcAddress)
    ...
    command = "curl -A HackDemoApplication -s http://%s/wm/device/?ipv4=%s" % (args.controllerRestIp, args.dstAddress)
    result = os.popen(command).read()
    parsedResult = json.loads(result)
    print command+"\n"
    ...
    command = "curl -A HackDemoApplication -s http://%s/wm/topology/route/%s/%s/%s/%s/json" % (controllerRestIp, sourceSwitch, sourcePort, destSwitch, destPort)
    result = os.popen(command).read()
    print result+"\n"
    print command+"\n"
    parsedResult = json.loads(result)
    ...
    for i in range(len(parsedResult)):
        if i % 2 == 0:
            ...
            command = "curl -A HackDemoApplication -s -d '{\"switch\": \"%s\", \"name\":\"%s\", \"ipv4_src\":\"%s\", \"ipv4_dst\":\"%s\", \"eth_type\":\"%s\", \"cookie\":\"0\", \"priority\":\"32768\", \"in_port\":\"%s\",\"active\":\"true\", \"actions\":\"output=%s\"}' http://%s/wm/staticflowpusher/json" % (ap1Dpid, ap1Dpid+"."+args.circuitName+".f", args.srcAddress, args.dstAddress, "0x800", ap1Port['shortPortNumber'], ap2Port['shortPortNumber'], controllerRestIp)
            result = os.popen(command).read()
            print command

            command = "curl -A HackDemoApplication -s -d '{\"switch\": \"%s\", \"name\":\"%s\", \"arp_spa\":\"%s\", \"arp_tpa\":\"%s\", \"eth_type\":\"%s\", \"cookie\":\"0\", \"priority\":\"32768\", \"in_port\":\"%s\",\"active\":\"true\", \"actions\":\"output=%s\"}' http://%s/wm/staticflowpusher/json" % (ap1Dpid, ap1Dpid+"."+args.circuitName+".farp", args.srcAddress, args.dstAddress, "0x806", ap1Port['shortPortNumber'], ap2Port['shortPortNumber'], controllerRestIp)
            result = os.popen(command).read()
            print command
        ...
        command="curl -A HackDemoApplication -s http://%s/wm/core/switch/all/flow/json| python -mjson.tool" % (controllerRestIp)
        result = os.popen(command).read()
        print command + "\n" + result
    ...
    for line in lines:
        ...
        if data['name']==(args.circuitName):
            ...
            command = "curl -A HackDemoApplication -X DELETE -d '{\"name\":\"%s\", \"switch\":\"%s\"}' http://%s/wm/staticflowpusher/json" % (sw+"."+args.circuitName+".f", sw, controllerRestIp)
            result = os.popen(command).read()
            print command, result

            command = "curl -A HackDemoApplication -X DELETE -d '{\"name\":\"%s\", \"switch\":\"%s\"}' http://%s/wm/staticflowpusher/json" % (sw+"."+args.circuitName+".farp", sw, controllerRestIp)
            result = os.popen(command).read()
            print command, result

            command = "curl -A HackDemoApplication -X DELETE -d '{\"name\":\"%s\", \"switch\":\"%s\"}' http://%s/wm/staticflowpusher/json" % (sw+"."+args.circuitName+".r", sw, controllerRestIp)
            result = os.popen(command).read()
            print command, result

            command = "curl -A HackDemoApplication -X DELETE -d '{\"name\":\"%s\", \"switch\":\"%s\"}' http://%s/wm/staticflowpusher/json" % (sw+"."+args.circuitName+".rarp", sw, controllerRestIp)
            result = os.popen(command).read()
            print command, result            
    ...
    print "This application is now hacked by a malicious third party.\nIt will send some unexpected commands to the controller."
    
    command = "curl -A HackDemoApplication -s http://%s/wm/staticflowpusher/json" % (controllerRestIp)
    
    command2 = "curl -A HackDemoApplication -s http://%s/wm/core/anomalies/json" % (controllerRestIp)
